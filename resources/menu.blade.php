				...
				<div class="card-body">
					...
					<div class="rounded mt-4 px-4 pt-4 pb-2" style="background-color: #e7ebee;">
						...
						<div class="card-title d-flex justify-content-center align-items-center text-center flex-wrap">
							...
							<h5 class="w-100 font-weight-normal" style="line-height: 0.8;"><?php echo $CompanyName; ?></h5>
						</div>

						<!-- php
						مقداردهی اولیه و تعیین گروه اصلی
						-->
						<?php
						$root = $c = $_REQUEST['c'];
						if ($c == "") {
							...
							$c = $c;
							$sql3 = mysqli_query($con, "SELECT * FROM menu where id='$c'");
							$row3 = mysqli_fetch_array($sql3);
							$CatName = $row3['stext'];
						}
						?>

						<!-- bazo bastan hame item ha -->
						<div class="d-flex align-items-center justify-content-between mb-4">
							<button type="button" class="btn btn-success mr-2" data-toggle="modal" data-target="#add_content_modal_Main">افزودن گروه اصلی</button>
						...
						<p class="text-muted">لیست تو در تو <i class="fa fa-angle-left mx-2"></i> <span style="font-size:1px; font-weight:bold; color:#2c3191;"><?php echo $CatName; ?></span></p>
						...
						<div class="dd" id="nestable1">
							<ol class="dd-list">
								...
								<!-- php
								بازگشتی (Recursive) برای ایجاد زیرشاخه‌ها
								-->
								<?php
								$dash = 0;
								function GetSubCat($pid)
								{
									...
									global $table;
									global $dataSub;
									$subsql = "SELECT * from menu where deltf='0' and c=" . $pid;
									$rs = mysqli_query($con, $subsql);
									$c = (int)mysqli_num_rows($rs);
									while ($res = mysqli_fetch_array($rs)) {
								?>
										<li class="dd-item dd3-item" data-id="<?php echo $res['id']; ?>">
											<div class="dd-handle dd3-handle"></div>
											<div class="dd3-content">
												<a data-toggle="modal" data-target=".edit_content_sub_modal" data-menuid="<?php echo $res['id']; ?>" data-menuroot="<?php echo $root; ?>" href="#">
													<figure class="avatar mr-2" style="width: 35px;height: 35px;">
														<?php
														if ($res['pic'] <> "") {
														?>
															<img src="./assets/media/image/have-image.jpg" class="rounded-circle" alt="<?php echo $res['stext']; ?>">
														<?php
														} else {
														?>
															<img src="./assets/media/image/no-image.jpg" class="rounded-circle" alt="image">
														<?php
														}
														?>
													</figure>
												...
												$sql20 = mysqli_query($con, "SELECT * FROM menu_details where cid = '$id' and deltf='0' ");
												$c20 = mysqli_num_rows($sql20);
												?>
												<div class="btns-options-tree">

													<button class="btn btn-success btn-sm dropdown-toggle btn-badge" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
														عملیات <span class="badge badge-secondary"><?php echo $c20; ?></span>
													</button>
													<div class="dropdown-menu">
														<a class="dropdown-item" data-toggle="modal" data-target=".add_content_modal_Sub" data-formurl="Menu/Menu-Insert.php?c=<?php echo $res['id']; ?>&&root=<?php echo $root; ?>" href="#"> افزودن زیر گروه</a>
														<a class="dropdown-item" data-toggle="modal" data-target=".edit_content_sub_modal" data-menuid="<?php echo $res['id']; ?>" data-menuroot="<?php echo $root; ?>" href="#"> ویرایش زیر گروه</a>

														<a class="dropdown-item" data-toggle="modal" data-target=".add_content_modal" data-menuid="<?php echo $res['id']; ?>" data-menuroot="<?php echo $root; ?>" href="#"> افزودن مطلب</a>
														<a class="dropdown-item" data-toggle="modal" data-target=".list_content_modal" data-menuid="<?php echo $res['id']; ?>" data-menuroot="<?php echo $root; ?>" href="#"> لیست مطلب</a>
														<a class="dropdown-item" href="#" data-toggle="modal" data-target=".modal_upload_files" data-formurl="<?php echo $res['id']; ?>">بارگذاری فایل</a>
														<a class="dropdown-item" href="#" data-toggle="modal" data-target=".modal_list_files" data-formurl="<?php echo $res['id']; ?>">لیست فایل های آپلود شده</a>
														<div class="dropdown-divider"></div>
														<div class="d-flex align-items-center justify-content-center">
															<button type="button" class="btn btn-warning" data-toggle="modal" data-target=".edit_content_sub_modal"><i class="fa fa-pencil"></i></button>
															<a href="#" data-formurl="Menu/Menu-Del.php?<?php echo $res['id']; ?>&&c=<?php echo $c; ?>" class="btn btn-danger sweet-del-content"><i class="fa fa-trash"></i></a>
														</div>
														<div class="dropdown-divider"></div>
														<a class="dropdown-item" href="#">(<?php echo $res['id']; ?>)</a>

													</div>
												</div>
											</div>
											<ol class="dd-list">
												<?php $dataSub .= GetSubCat($id); ?>
											</ol>
										</li>
									...
									$table = "menu";
									$subtable = "menu_detalis";

									$con = mysqli_connect('localhost', 'fenixco_CRM', 'Sohrab@1234');
									mysqli_select_db($con, 'fenixco_CRM');
									mysqli_set_charset($con, 'utf8');

									// داده‌های گروه اصلی (Root) از جدول menu دریافت می‌شود.
									$sql = mysqli_query($con, "SELECT * from $table where c = '$c' and deltf='0' order by id asc ");
									...
									?>
										...
										<li class="dd-item dd3-item" data-id="<?php echo $rs['id']; ?>">
											<div class="dd-handle dd3-handle"></div>
											<div class="dd3-content">
												<a data-toggle="modal" data-target=".edit_content_sub_modal" data-menuid="<?php echo $rs['id']; ?>" data-menuroot="<?php echo $root; ?>" href="#">
													<figure class="avatar mr-2" style="width: 35px;height: 35px;">
														<?php
															if ($rs['pic'] <> "") {
														?>
															<img src="./assets/media/image/have-image.jpg" class="rounded-circle" alt="<?php echo $rs['stext']; ?>">
														<?php
															} else {
														?>
															<img src="./assets/media/image/no-image.jpg" class="rounded-circle" alt="image">
												...
												<?php echo $stext = $rs['stext']; ?>/<?php echo $stext = $rs['enstext']; ?>
												<div class="btns-options-tree">
													<button class="btn btn-success btn-sm dropdown-toggle btn-badge" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
														عملیات <span class="badge badge-secondary"><?php echo $c20; ?></span>
													</button>
													<div class="dropdown-menu">
														<a class="dropdown-item" data-toggle="modal" data-target=".add_content_modal_Sub" data-formurl="Menu/Menu-Insert.php?c=<?php echo $rs['id']; ?>&&root=<?php echo $root; ?>" href="#"> افزودن زیر گروه</a>
														<a class="dropdown-item" data-toggle="modal" data-target=".edit_content_sub_modal" data-menuid="<?php echo $rs['id']; ?>" data-menuroot="<?php echo $root; ?>" href="#"> ویرایش زیر گروه</a>

														<a class="dropdown-item" data-toggle="modal" data-target=".add_content_modal" data-menuid="<?php echo $rs['id']; ?>" data-menuroot="<?php echo $root; ?>"> افزودن مطلب</a>
														<a class="dropdown-item" data-toggle="modal" data-target=".list_content_modal" data-menuid="<?php echo $rs['id']; ?>" data-menuroot="<?php echo $root; ?>" href="#"> لیست مطلب</a>
														<a class="dropdown-item" href="#" data-toggle="modal" data-target=".modal_upload_files" data-formurl="<?php echo $rs['id']; ?>">بارگذاری فایل</a>
														<a class="dropdown-item" href="#" data-toggle="modal" data-target=".modal_list_files" data-formurl="<?php echo $rs['id']; ?>">لیست فایل های آپلود شده</a>
														<div class="dropdown-divider"></div>
														<div class="d-flex align-items-center justify-content-center">
															<button type="button" class="btn btn-warning" data-toggle="modal" data-target=".edit_content_sub_modal"><i class="fa fa-pencil"></i></button>
															<a href="#" data-formurl="Menu/Menu-Del.php?id=<?php echo $rs['id']; ?>&&c=<?php echo $c; ?>" class="btn btn-danger sweet-del-content"><i class="fa fa-trash"></i></a>
														</div>
														<div class="dropdown-divider"></div>
														<a class="dropdown-item" href="#">(<?php echo $rs['id']; ?>)</a>
													</div>
												</div>
											</div>

											<ol class="dd-list">
												<!-- هر آیتم سطح بالا (سطح ریشه) با زیرشاخه‌هایش توسط تابع GetSubCat پر می‌شود. -->
												<?php $data .= GetSubCat($id); ?>
											</ol>
										</li>
									<?php
									}
	...
	$(document).ready(function() {
		...
		$('.list_content_modal').on('shown.bs.modal', function(event) {
			...
			$.ajax({
				type: 'POST',
				url: 'List.php',
				data: "MenuContentId=" + MenuContentId + "&&MenuContentRoot=" + MenuContentRoot + "&&DetailsContentid=" + DetailsContentid,
			...
			$.ajax({
				...
				success: function(data) {
					...
					$('#imageUpload3').on('change', function() {
						...
						var file = document.querySelector('input[id=imageUpload3]').files[0];
					...
					$('#imageUpload4').on('change', function() {
						var file = document.querySelector('input[id=imageUpload4]').files[0];
						var reader = new FileReader();
					...
					$('.btn-del-imagefile').on('click', function() {
						$(this).parents('.avatar-upload').find('.avatar-preview img').attr('src', './assets/media/image/no-image.jpg');
					...
					$('.btn-del-videofile').on('click', function() {
						$(this).parents('.avatar-upload').find('.avatar-preview video').attr('poster', './assets/media/image/no-video.jpg');
						$(this).parents('.avatar-upload').find('.avatar-preview video').attr('src', '');
						$(this).parents('.avatar-upload').find('.avatar-preview video').removeAttr('controls');
						$(this).css('display', 'none');
					});
			...
			$.ajax({
				...
				success: function(data) {
					...
					$('#imageUpload5').on('change', function() {
						// var preview = $('#avaterupload2 img');
						var file = document.querySelector('input[id=imageUpload5]').files[0];
						var reader = new FileReader();
					...
					$('#imageUpload6').on('change', function() {
						// var preview = $('#avaterupload2 img');
						var file = document.querySelector('input[id=imageUpload6]').files[0];
						var reader = new FileReader();
